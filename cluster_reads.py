...
def findreleations(thereadid, reads, mink, usedreads, readsdic):
...
def cluster(filename, mink):
    ...
    for i in range(0, len(reads) - 1):
        if i not in usedreads:
            s = findreleations(i, reads, mink, usedreads, readsdic)
            relations.append(s)
